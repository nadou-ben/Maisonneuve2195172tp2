...
<div class="container mt-5">
    <form  method="post" enctype="multipart/form-data">
      <h3 class="text-center mb-5">@lang('lang.text_edit_doc')</h3>
        @csrf
        @method('PUT')
      ...
      <label >@lang('lang.text_add_doc_en'):</label>
        <div class="custom-file  mb-4">
          ...
          <label class="custom-file-label" for="customFile">{{$file->titre}} </label>
          
        </div>
        <label >@lang('lang.text_add_doc_fr'): </label>
        <div class="custom-file">
          <input type="file" name="titre_fr"  class="custom-file-input" id="customFile"  >
          <label class="custom-file-label" for="customFile">{{$file->titre_fr}}</label>
      </div>
        <button type="submit" name="submit" class="btn btn-primary btn-block mt-4">
          @lang('lang.text_edit_doc')
        </button>
